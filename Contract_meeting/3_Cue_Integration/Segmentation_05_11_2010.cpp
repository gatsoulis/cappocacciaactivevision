...
//	 showImg(IBinary, "postBinary");
	...
	for(int m=0; m<IBinary->height;m++)
	{
		...
		{ 
			...
			if(CComp.area<9000 && CComp.area>900 ) // considering at least 30x30 pixel size object
			{
				Object obj;
				float d=0.0;
				bool bFindObj=false;
				...
				if(false==bFindObj && CComp.area<9000 && CComp.area>900)
				{
...
//	showImg(IF_Likelihood,"if");
	cvThreshold( IF_posterior, IBinary, 0.45,255,CV_THRESH_BINARY );
...
*/	
	...
	for(int j = 0;j<IBinary->height;j++)
	{
		...
		//	if( ( (float)CComp.rect.height/(float)CComp.rect.width) >=0.5 && ( (float)CComp.rect.height/(float)CComp.rect.width) < 2.0 && CComp.area>900 && ( ( (float)CComp.area/(float)(CComp.rect.height*CComp.rect.width) ) >0.4 ) ) // considering at least 30x30 pixel size object
			if( CComp.area>900 && CComp.area<9000 ) // considering at least 30x30 pixel size object
			{
				NoConnComp++;
			}
			else
...
//	IplImage *temp=cvCloneImage(IBinary);
	...
	for(int j = 0;j<IBinary->height;j++)
	{
		...
		{ 
			cvFloodFill(IBinary,pt, cvScalar((NoConnComp+1)*2), cvScalarAll(0), cvScalarAll(0),&(CComp), Connectivity, NULL );
 			if(CComp.area>500 && CComp.area<7000 && ( ( (float)CComp.area/(float)(CComp.rect.height*CComp.rect.width) ) >0.5 ) ) // considering at least 30x30 pixel size object
			{
				NoConnComp++;
				break;
			}
