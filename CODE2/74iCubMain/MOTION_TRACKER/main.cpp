...
#define CTRL_THREAD_PER     0.02        // [s]0.02
		...
		if (state==STATE_TRACK)
		{
			...
			if (targetIndL != NULL && targetIndL->size()==4 && targetIndR != NULL && targetIndR->size()==4)
			{
				...
				if(confL != 0 && confR != 0)
				{
					...
					else if((distL<=25.0 && distR<=25.0) || (TypeL == "LookAtLocation" && TypeR == "LookAtLocation"))
					{
...
*/	
	...
	*/
	...
	virtual double getPeriod()    { return 0.02;}//1.0;  }
	...
	{ 
		...
		if(thr->movehome==true)
		{
			printf("\nStart move home...\n");
			encs->getEncoder(1,&joint1);
			if(joint1 <= (thr->startroll + 0.3) && joint1 >= (thr->startroll -0.3) )
			{	
