...
CueContrastKernel::CueContrastKernel(std::string name) : Cue(name) {
...
CueContrastKernel::~CueContrastKernel() {
	...
	if(mp_cvminimg) delete mp_cvminimg;
	if(mp_cvmaximg) delete mp_cvmaximg;

	std::cout << "finished.\n";
}

void CueContrastKernel::setParam(int opmode, int maxIter, std::vector<int>* binSize, std::vector<int>* contrastScale) {
//	if(debug) std::cout << getName() << "::setParam()::Started\n";
	...
	if(binSize) {
		...
		m_vecbinsize.push_back(32);
		m_vecbinsize.push_back(32);
	}

	if(contrastScale) {
		if(contrastScale->size() != m_dimData) {
			std::cerr << getName() << "::setParam()::ERROR::binSize dim does not match contrastmode!\n";
			return;
		}

		m_contrastscale = *contrastScale;
	}
	else{
		m_contrastscale.push_back(5);
		m_contrastscale.push_back(11);
	}
...
float CueContrastKernel::computeSimilarity(CSCVector* p, CSCVector* q) {
...
void CueContrastKernel::drawGrayBall(CvPoint2D32f* pos, CvSize* size) {
	float temp = 1.0;
...
int CueContrastKernel::findIndex(unsigned char* input) {
	int temp, result;
	for(unsigned int i =0;i<m_dimData;i++){ 
		temp = (int)(input[i]);
...
float CueContrastKernel::computeKernel(float dist) {
...
void CueContrastKernel::computeWeights(CSCVector* p) {
...
void CueContrastKernel::findNextLocation(CvPoint2D32f* pos, float hval) {
	// Step 3
...
bool CueContrastKernel::checkCondition(CvPoint2D32f* pos, CvPoint2D32f* nextpos, int count) {
...
int CueContrastKernel::findmax(float* a, int size) { 
	int index = 0;
	float temp = a[0];
...
/*
float CueContrastKernel::similarityFunction(float* p, float* q, int size) {
	float result = 0.0;
...
float CueContrastKernel::distance(float* p, float* q, int size) {
