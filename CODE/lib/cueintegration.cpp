...
CueIntegration::CueIntegration(std::string name) : Module(name) {
	std::cout << "\nConstructing " << getName() << "...";

/*
	targetPortLeft.open("/tutorial/Left/target/out");
*/
...
void CueIntegration::connect(){

	PointCueOutPort.open("/cueintegration/left/out");
	Network::connect("/cueintegration/left/out","/motion/left/in");

}
void CueIntegration::clean(){

	std::cout<<"\nDisconnetting /cueintegration/left/out ...";
...
void CueIntegration::setthres(float val) {
	m_threshold = val;
/*
...
void CueIntegration::addCue(Cue* newcue, float reliability) {
...
void CueIntegration::initialize() {
...
void CueIntegration::obtainInput() {
...
//	if(debug) std::cout << getName() << "::execute()::ret = " << ret << "\n";
	...
	xL = m_track.winnerPos.x;
	yL = m_track.winnerPos.y;
	std::cout<<"xl : "<< xL <<"\n";
	std::cout<<"yl : "<< yL <<"\n";

	/*point = cvPoint(xL,yL);
	cvpointOutL.setBuffer(&point);
	cvpointOutL.out();*/

	/*Vector& targetL = PointCueOutPort.prepare();
        targetL.resize(3);
        targetL[0] = xL;
        targetL[1] = yL;
        targetL[2] = 1;
	...
	Bottle& targetL = PointCueOutPort.prepare();
        targetL.clear();
...
void CueIntegration::computesaliencymap()  {
...
void CueIntegration::computequality() {
...
void CueIntegration::computereliability() {
...
#ifdef USE_THREADS
void *CueIntegration::compute1(void* ptr) {
	CueIntegration *obj = reinterpret_cast<CueIntegration*>(ptr);
...
void *CueIntegration::compute2(void* ptr) {
	CueIntegration *obj = reinterpret_cast<CueIntegration*>(ptr);
...
void *CueIntegration::compute3(void* ptr) {
	CueIntegration *obj = reinterpret_cast<CueIntegration*>(ptr);
...
void *CueIntegration::compute4(void* ptr) {
	CueIntegration *obj = reinterpret_cast<CueIntegration*>(ptr);
...
void *CueIntegration::compute5(void* ptr) {
	CueIntegration *obj = reinterpret_cast<CueIntegration*>(ptr);
...
void *CueIntegration::compute6(void* ptr) {
...
void *CueIntegration::compute7(void* ptr) {
...
void *CueIntegration::compute8(void* ptr) {
