#ifndef CueColorKernel_H
#define CueColorKernel_H
...
//! class CueColorKernel  -- Color cue based on local color histogram. (adaptive?)
/*! class CueColorKernel  -- Color cue based on local color histogram.

		written 03/2005 by Hyundo Kim
*/
	class CueColorKernel : public Cue
	{

		public: 

			typedef enum {SIMPLE=0, WBGHIST=1, WKALMAN=2, BGHISTKALMAN=4} operation_modes;
			typedef enum {USE_HS=0, USE_HSV=1, USE_RGB=2} color_modes;

			CueColorKernel(std::string name = "CueColorKernel");
			~CueColorKernel();

			void execute();
			void adapt();
			void initialize();
			void setParam(int opmode = SIMPLE, int colormode = USE_RGB, int maxIter = 20, std::vector<int>* binSize = NULL);
			bool debug;
